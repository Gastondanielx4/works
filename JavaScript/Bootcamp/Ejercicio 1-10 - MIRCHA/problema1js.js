...
cadenaAArreglo ("Estoy haciendo el ejercicio uno con jon mircha en la escuelas", " ")
...
/*  4) Programa una función que repita un texto X veces, pe. miFuncion('Hola Mundo', 
 ...
 if(veces === undefined) return console.warn("No ingresaste las veces de repetición");
...
/* 
 ...
 let cuentaCoincidencias = (fraseAContar="", textoABuscar = "") => {
   ...
   let i = 0,
   contador = 0;
  while(i !== -1){ //cuando indexOf no encuentra da la posicion -1, por eso esta condicion para que evalue esto antes
...
const palindromo = (palabre = "") => {
   ...
   ?console.info(`Si es palindromo, palabra original "${palabre}", palabra al revés "${alReves}"`)
   :console.info(`No es palindromo,palabra original "${palabre}", palabra al revés "${alReves}"`)
}
palindromo();
...
let numeroFactorial = (numeroAFactorear= undefined) =>{
   if (numeroAFactorear === undefined) return console.warn("No Ingresaste un numero");
   if(typeof numeroAFactorear !== "number") return console.error(`El valor "${numeroAFactorear}" ingresado, NO es un numero`);
   if (numeroAFactorear === 0) return console.error ("El numero no puede ser 0");
   if (Math.sign(numeroAFactorear) === -1) return console.error("No puede ser negativo");
...
let numeroPrimo = (numeroP) => {
      ...
      if(typeof numeroP !== "number") return console.error(`El valor "${numeroP}" ingresado, NO es un numero`);
...
let parOImpar = (serapar) =>{
   if(typeof serapar !== "number") return console.error(`El valor "${serapar}" ingresado, NO es un numero`)
   return ((serapar%2)=== 0)
...
let convertirGrados = (grados= undefined, unidad= undefined ) =>{
   if(grados === undefined) return console.error("No ingresaste datos a convertir");

   if(typeof grados !== "number") return console.error("No ingresaste numeros");

   if(typeof unidad !== "string") return console.error("No ingresaste una string");
...
//15) Programa una función para convertir números de base binaria a decimal y viceversa,
 ...
 let convertidorBinario = (numero = undefined, base=undefined) =>{
    ...
    if (typeof base!== `number`) return console.warn("Valor de sistema no reconocido");
...
let aplicarDescuento = (cifra, descuento) => {
   if (descuento === 0) console.log (`El producto se debe pagar a ${cifra} ya que es sin descuento`)
...
let nombreValido = (nombre, apellido) => {
   expRegNombre = /^([A-Z]{1}[a-zñáéíóú]+[\s]*)+$|^([A-Z]{1}[a-zñáéíóú]+[\s]*)+$[\s]([A-Z]{1}[a-zñáéíóú]+[\s]*)+$ /g;
   expRegApellido = /^([A-Z]{1}[a-zñáéíóú]+[\s]*)+$/g;
   if(expRegNombre.test(nombre) === false) { return console.warn("El nombre ingresado no es valido")};
   if(expRegApellido.test(apellido)=== false) {return console.warn("El apellido ingresado no es valido")};
...
let arrayAlCuadrado = (arr = undefined) =>{
   ...
   for (let num of arr) { //se usa for of porque el array es un objeto iterable
      if(typeof num !== "number") return console.error("El valor no es un numero");
   }
...
//22) Programa una función que dado un array devuelva el número mas alto y el más bajo
 ...
 let arrayMinMax = (arr = undefined) =>{
   ...
   return console.info (`Arreglo Original: ${arr}\nValor Mayor: ${Math.max(...arr)} Valor menor${Math.min(...arr)}`)
...
pe. miFuncion([1,2,3,4,5,6,7,8,9,0]) devolverá {pares: [2,4,6,8,0],
   ...
   const ordenarNumeros = (arr = undefined) => {
      ...
      return console.info({
            ...
            numerosDescendentes: arr.map(el => el).sort().reverse(),
         }
      );
   }
   ordenarNumeros([1, 4, 2, 9, 10, 11, 3])
    ...
    const quitarDuplicados = (arr = undefined) => {
      ...
      if(arr.length === 1 ) return console.error("El arreglo necesita dos elementos");
...
En el video 43 te daré mi solución, antes de verlo trata de resolver los ejercicios,
 y comparte tus resultados en alguna plataforma como GitHub o CodePen y comparte el
  ...
  class Pelicula {
     constructor({id, titulo, director, añoEstreno, paisDeOrigen, generos, calificacion}){
        this.id = id;
        this.titulo = titulo;
      ...
      validarArreglo(propiedad, valor){
         ...
         for (let cadena of valor) { //se usa for of porque el array es un objeto iterable
      ...
      validarIMDB(id){
         if(this.validarCadena("IMDB id", id))
          if(!(/^([a-z]){2}([0-9]{7})$/.test(id))){
             return console.error(`IMDB id "${id} no es valido, debe tener 9 caracteres, los 2 primero letras minusculas, los 7 restantes numeros`);
          }
      }
      ...
      validarAñoEstreno(añoEstreno){
          if(this.validarNumero("Año de Estreno", añoEstreno))
            if(!(/^([0-9]){4}$/.test(añoEstreno)))
             return console.error(`Año de Estreno "${añoEstreno} no es valido, debe tener 4 digitos.`);
      }
